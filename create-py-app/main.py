...
def main():
    ...
    with open('file_contents.yml', 'r') as f:
        ...
        create_file(base_dir, "environment_variables.py", content=yaml_data["environment_variables.py"])
        create_file(base_dir, ".env", content=yaml_data[".env"])
        create_file(base_dir, ".pre-commit-config.yaml", content=yaml_data[".pre-commit-config.yaml"])
